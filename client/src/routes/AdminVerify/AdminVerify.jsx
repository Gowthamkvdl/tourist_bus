...
const AdminVerify = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["adminPosts"], // Unique key for caching
    queryFn: async () => {
    ...
    <div className="">
      <div className="header pt-md-1">
        ...
        <div className="head d-flex flex-column my-3 mt-4 justify-content-center align-items-center gap-1">
          <h2 className="text-center fw-bold">Verify Buses</h2>
          <p className="p-0 m-0">Verification pending buses will appear here</p>
        </div>
      </div>
      ...
      <div className="others box-shadow bg-white py-md-5 py-3 px-2 px-md-4">
        <div className="cards row px-md-4 px-3 mb-5">
          {isLoading ? (
            ...
            data.postData
              .filter((post) => post.verificationStatus === "pending")
              .map((post) => (
